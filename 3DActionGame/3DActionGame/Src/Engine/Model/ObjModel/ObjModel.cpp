...
void ObjModel::ParseFKeywordTag(MeshData& meshData_, std::vector<DirectX::XMFLOAT3>& vertices_, std::vector<DirectX::XMFLOAT3>& textures_, std::vector<DirectX::XMFLOAT3>& normals_, char* buffer_)
{
    ...
    {
        int temp = meshData_.Indices[size - 1];
        meshData_.Indices[size - 1] = meshData_.Indices[size - 4];
        meshData_.Indices[size - 4] = temp;

        temp = meshData_.Indices[size - 2];
        meshData_.Indices[size - 2] = meshData_.Indices[size - 3];
        meshData_.Indices[size - 3] = temp;
    ...
    //ポリゴンの作成の頂点順番を反転する
    UINT temp = meshData_.Indices[size - 1];
    meshData_.Indices[size - 1] = meshData_.Indices[size - 3];
    meshData_.Indices[size - 3] = temp;
}
