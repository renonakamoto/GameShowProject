...
Kabochaten::Kabochaten(D3DXVECTOR3 pos_, const ObjectBase* player_, std::string key_) :
	Enemybase(pos_, player_, key_), m_CrrentMotion(KabochaMotionList::Wait)
{
	m_Motion.AddMotion(KabochaMotionList::Wait, 0, 60);
	m_Motion.AddMotion(KabochaMotionList::Walk, 70, 130);
	m_Motion.AddMotion(KabochaMotionList::Sprint, 140, 200);
	m_Motion.AddMotion(KabochaMotionList::LookAround, 210, 270);
	m_Motion.AddMotion(KabochaMotionList::ChangeDirection, 280, 340);

	m_Shape.push_back(new AABBShape(m_Pos, 4.55f, 11.2f, 18.2f));

	Navigator::GetInstance().GetEnemyRoute("Kabocha", m_PatrolRoute);
	m_NextRoute = m_PatrolRoute.front();
	m_State = StateManager::GetInstance()->GetState(StateType::Patrol);
	m_Type = Objectmanager::EnemyType::Enemy_Kabochaten;
}
...
void Kabochaten::Patrol()
{
	...
	m_Motion.Motion(KabochaMotionList::Walk, m_FbxKey, true);
}

void Kabochaten::Turn()
{
	if (CanDetectPC() == true)
	{
		m_State = StateManager::GetInstance()->GetState(StateType::Chase);
	...
	m_Motion.Motion(KabochaMotionList::Wait, m_FbxKey, true);
}

void Kabochaten::Chase()
{
	...
	m_Motion.Motion(KabochaMotionList::Sprint, m_FbxKey, true);
}

void Kabochaten::Return()
{
	if (CanDetectPC() == true)
	{
		m_State = StateManager::GetInstance()->GetState(StateType::Chase);
